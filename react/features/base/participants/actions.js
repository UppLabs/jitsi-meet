...
export function localParticipantJoined(participant = {}) {
    const _participant = { ...participant, local: true, parsedName: (participant.name || "").split("::") }
    return participantJoined(_participant);
}
...
export function participantJoined(participant) {
    ...
    let _participantData = { ...participant };

    const { name, avatarURL, local, parsedName } = _participantData;
